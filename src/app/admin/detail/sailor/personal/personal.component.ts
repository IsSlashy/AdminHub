...
import { SAILOR_PERSONNAL, UPDATE_DETAIL_SAILOR, UPDATE_SAILOR } from 'src/graphql/sailor';
    ...
    this.route.parent?.paramMap.subscribe((param) => {
      ...
      this.subscription = this.apollo.watchQuery<any>({
        ...
        this.personalForm.patchValue({
          ...
          formattedAddress: data.user.userDetailById.personnalAddress?.formattedAddress,
  ...
  updateUser(){
    this.apollo.mutate({
      ...
      this.apollo.mutate({
        ...
        refetchQueries: [{query:SAILOR_PERSONNAL, variables:{userId: this.userId}}],
        variables: {
          sailorPayload: {
            id: this.userId,
            patch: {
              ...
              socialSecurityNumber: this.personalForm.value.socialSecurityNumber,
