...
import { SAILOR_PROFIL, UPDATE_LANGAGE, UPSERT_PREF, UPSERT_VISAS } from 'src/graphql/sailor';
...
export class ProfilComponent {
  ...
  ngOnInit(){
    this.route.parent?.paramMap.subscribe((param) => {
      ...
      this.subscription = this.apollo.watchQuery<any>({
        ...
        const visas = data.user.userVisas?.nodes.map((elt: any) => elt.visa.id);
        const langs = data.user.sailorSpokenLanguages?.nodes.map((elt: any) => elt.language.id);
        const boats = data.user.userPreference?.userPreferedBoatTypes.nodes.map((elt: any) => elt.boatType);
        const ads = data.user.userPreference?.userPreferedAdTypes.nodes.map((elt: any) => elt.adType);
        const positions = data.user.userPreference?.userPreferedPositions.nodes.map((elt: any) => elt.position);
        ...
        this.profilForm.patchValue({
          ...
          contractType: data.user.userPreference?.userPreferedContractType?.contractType,
 ...
 goToUrl(type:any){
  window.open('https://www.captnboat.com/fr/recherche/marins-professionels?id=' + this.userId)
 }
