...
import { JOB, UPDATE_AD, UPDATE_JOB, UPDATE_SPOKENN_LANGUAGE } from 'src/graphql/job';
    ...
    this.route.parent?.paramMap.subscribe((param) => {
      ...
      if (this.jobId) {
        this.apollo.query({
          ...
          const lang = data.job.ad.spokenLanguagesByAdsId?.nodes.map((elt: any) => elt.language.id);
          this.jobForm.patchValue({
            ...
            endDate: moment.tz(data.job.endDate, 'Europe/Paris').toISOString(),
            ...
            startDate: moment.tz(data.job.startDate, 'Europe/Paris').toISOString(),
      ...
      variables: {
        jobInput: {
          ...
          patch: {
            ...
            endDate: moment.tz(this.jobForm.value.endDate, 'Europe/Paris').format(),
            ...
            sleepOnBoardNightBefore: this.jobForm.value.sleepOnBoardNightBefore,
            sleeping: this.jobForm.value.sleeping,
            jobStatus: this.jobForm.value.jobStatus,
            startDate: moment.tz(this.jobForm.value.startDate, 'Europe/Paris').format(),
