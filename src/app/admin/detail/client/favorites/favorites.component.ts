...
import { VIEW_FAVOURTIES, ADD_FAVOURITES, DELETE_FAVOURITE } from 'src/graphql/favoris';
...
export class FavorisComponent implements OnInit {
  ...
  addFavourite(favouriteUserId: string): void {
    ...
    this.apollo.mutate({
      ...
      refetchQueries: [{ query: VIEW_FAVOURTIES, variables: { favoriteUser: this.userId } }]
    }).subscribe({
      next: (response) => {
        console.log('Favori ajouté', response);
        this.newFavouriteId = ''; // Réinitialiser l'ID du nouveau favori
      },
      error: (error) => console.error('Erreur lors de l\'ajout du favori', error)
    });
  }
  ...
  deleteFavourite(favouriteId: string): void {
    this.apollo.mutate({
      ...
      error: (error) => console.error('Erreur lors de la suppression du favori', error)
    });
  }
}
