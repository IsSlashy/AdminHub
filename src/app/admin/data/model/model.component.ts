...
import { CREATE_BOATMODEL, GET_BOATMODEL, UPDATE_BOATMODEL } from 'src/graphql/boats';
    ...
    if (this.boatForm.valid) {
      if (this.isEditing) {
        this.apollo.mutate({
          ...
          console.error('Erreur lors de la mise à jour du modèle de bateau', error);
        ...
        this.apollo.mutate({
          ...
          console.error('Erreur lors de la création du modèle de bateau', error);
          this.modalConfirmed.modalRejected();  // Ouvrir le modal en cas d'erreur
        });
      }
    }
