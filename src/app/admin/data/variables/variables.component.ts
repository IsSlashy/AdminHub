...
export class VariablesComponent implements OnInit {
  ...
  ngOnInit() {
    this.apollo.watchQuery({ query: QUERY_VARIABLE }).valueChanges.subscribe(result => {
  ...
  update_Variable(){
    if (this.variableid) {
      this.apollo.mutate({
        ...
        refetchQueries:[{query: QUERY_VARIABLE, variables:{id: this.variableid}}],
        variables:{
          variableInput: {
            id: this.variableid,
            patch: {
              ...
              categorie12Jour: parseFloat(this.editForm.value.categorie12Jour),
              categorie12Base: parseFloat(this.editForm.value.categorie12Base) ,
