...
import { DELETE_RESUME, GENERATE_URL, GET_NO_RESUME, GET_RESUMES, REFUSE_DOCUMENT, REFUSE_RESUME, UPLOAD_RESUME, VALIDE_RESUME } from 'src/graphql/approval';
...
openModal(resume: any) {
  ...
  if(resume){
    ...
    this.urlDocument = this.sanitizer.bypassSecurityTrustResourceUrl(this.selectedResume.url)
...
handleFileInput(event: Event,userId:string) {
  ...
  this.apollo.mutate({
    ...
    console.log('le url', data.generatePresignedPost.url + '/' + data.generatePresignedPost.fields.key)
    this.apollo.mutate({
