...
export class GooglePlacesAutocompleteComponent {
  ...
  handleAddressChange(address: any) {
    const country = address.address_components.find((x: any) => x.types[0] === 'country');
    const county = address.address_components.find((x: any) => x.types[0] === 'administrative_area_level_1');
    const zip = address.address_components.find((x: any) => x.types.some((e:string) => ['postal_code', 'postal_town'].includes(e)));
    const city = address.address_components.find((x: any) => x.types.some((e:string) => ['administrative_area_level_2', 'locality'].includes(e)));
    const street = address.address_components.find((x: any) => x.types[0] === 'route');
    const number = address.address_components.find((x: any) => x.types.some((e:string) => ['premise', 'street_number'].includes(e)));
